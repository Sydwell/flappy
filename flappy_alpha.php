    ...
    <body id='theBody' onkeydown='keyHandler();' onclick="initialize();" style='padding:0px; margin:0px;'>
        <div style="float:right; margin-right:100px;">
            ...
            <table id="leaderBoard" style="border: #000099 solid thick;">
                ...
                $p_q = "SELECT   aka, surname, game_scores_t.uid, points,`date` from game_scores_t 
LEFT JOIN user_t as u on u.uid = game_scores_t.uid
where points= (select max(points) from game_scores_t as g where g.uid=game_scores_t.uid) group by uid order by points desc;";
                $theData = phpSelect($p_q, "", "flappy_alpha.php");
                $data = json_decode($theData);
                $ten = 0;
                foreach ($data as $key => $value) {
                    if ($ten > 10)
                        break;
                    $ten++;
                    echo(" <tr><td>$value[0]</td><td>$value[1]</td><td>$value[3]</td><td>$value[4]</td></tr>");
                }
                ?>
